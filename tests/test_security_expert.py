...
""")
    ...
    @pytest.mark.asyncio
    async def test_detect_hallucinations_with_credentials(self, security_expert, tmp_path):
...
""")
    ...
    @pytest.mark.asyncio
    async def test_detect_hallucinations_with_subprocess(self, security_expert, tmp_path):
...
""")
    ...
    async def test_suggest_fixes(self, security_expert):
        ...
        assert len(fixes) == 2
        assert fixes[0]["fix"] == "Replace hardcoded credential with environment variable"
